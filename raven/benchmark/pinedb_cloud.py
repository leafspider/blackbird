...
def upsert_records(pc, index_name, namespace, records):
...
def query(pc, index_name, namespace, top_k, query_text):
...
def fetch_tweets_as_records(topics):
...
if __name__ == '__main__':
    ...
    for namespace in namespaces:    
        records = fetch_tweets_as_records( topics=namespace['topics'])    
        upsert_records(pc, index_name, namespace['name'], records)
    print("upsert", time.time() - t0)
    
    t0 = time.time()    
    for namespace in namespaces:
        results = query(pc, index_name, namespace['name'], top_k=5, query_text="Which technology will have biggest possible societal impact in the near future?")
