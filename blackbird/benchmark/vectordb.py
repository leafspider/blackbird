...
class PostgresDb:
    ...
    def insert(self, table, **kwargs):
        ...
        # sql: str = f"""INSERT INTO {table} ({keys}) VALUES ({values});"""
        ...
        try:
            ...
            # with self.connect() as conn:
            #     with conn.cursor() as cur:
            #         ret = execute_values(cur, sql, values, fetch=True)
            #         # ret = cur.statusmessage
            #         # ret = cur.fetchone()
            #     conn.commit()
    ...
    def resequence(self, table, id_column, sequence):
        
        sql: str = f"""SELECT setval('{sequence}', (SELECT MAX({id_column}) FROM {table}));"""
        return self.execute(sql)
